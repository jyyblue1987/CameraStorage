...
function loqozero_menu($array, $child = FALSE, $lang_code){
    $CI =& get_instance();
    $str = '';
    ...
    if (count($array)) {
        ...
        //    $active = $CI->uri->segment(2) == url_title_cro($item['id'], '-', TRUE) ? TRUE : FALSE;
            $active = $CI->uri->segment(2) == url_title(convert_accented_characters($item['id']), 'dash', TRUE) ? TRUE : FALSE;
            ...
            if (isset($item['children']) && count($item['children'])) {
                $str .= $active ? '<li class="dropdown active">' : '<li class="dropdown1">';
                $str .= '<a  class="dropdown-toggle" data-toggle="dropdown" href="' . site_url($lang_code.'/'.$item['id'].'/'.url_title_cro($item['title'], '-', TRUE)) . '"><i class="fa fa-plus"></i>' . $item['title'];
                $str .= '<b class="caret"></b></a>' . PHP_EOL;
                $str .= get_menu($item['children'], TRUE, $lang_code);
            }
            else {
                $str .= $active ? '<li class="active">' : '<li>';
            //    $str .= '<a href="' . site_url($lang_code.'/'.$item['id'].'/'.url_title_cro($item['navigation_title'], '-', TRUE)) . '"><i class="fa fa-plus"></i>' . $item['navigation_title'] . '</a>';
                $str .= '<a href="' . site_url($lang_code.'/'.$item['id'].'/'.url_title(convert_accented_characters($item['title']), 'dash', TRUE)) . '"><i class="fa fa-plus"></i>' . $item['title'] . '</a>';
            }
            $str .= '</li>' . PHP_EOL;

        }
    //  $str .='<li><a href="'.base_url('frontend/gallery').'"><i class="fa fa-plus"></i>Gallery</a></li>'  . PHP_EOL;
        $str .= '</ul>' . PHP_EOL;
    }
...
function get_menu($array, $child = FALSE, $lang_code)
{
    ...
    if (count($array)) {
        ...
        //    $active = $CI->uri->segment(2) == url_title_cro($item['id'], '-', TRUE) ? TRUE : FALSE;
            ...
            if($key == 1){
            //    $item['navigation_title'] = '<img src="assets/img/home-icon.png" alt="'.$item['navigation_title'].'" />';
                $item['navigation_title'] = $item['navigation_title'];
                $active = TRUE;
            }
            
            if (isset($item['children']) && count($item['children'])) {
                $str .= $active ? '<li class="dropdown active">' : '<li class="dropdown1">';
                $str .= '<a  class="dropdown-toggle" data-toggle="dropdown" href="' . site_url($lang_code.'/'.$item['id'].'/'.url_title_cro($item['title'], '-', TRUE)) . '">' . $item['title'];
                $str .= '<b class="caret"></b></a>' . PHP_EOL;
                $str .= get_menu($item['children'], TRUE, $lang_code);
            }
            else {
                $str .= $active ? '<li class="active">' : '<li>';
            //    $str .= '<a href="' . site_url($lang_code.'/'.$item['id'].'/'.url_title_cro($item['navigation_title'], '-', TRUE)) . '"><i class="fa fa-plus"></i>' . $item['navigation_title'] . '</a>';
                $str .= '<a class="dropdown-toggle" href="' . site_url($lang_code.'/'.$item['id'].'/'.url_title(convert_accented_characters($item['title']), 'dash', TRUE)) . '">' . $item['title'] . '</a>';
            }
            $str .= '</li>' . PHP_EOL;
...
function get_lang_menu ($array, $lang_code)
{
    ...
    foreach ($array as $item) {
        ...
        if($CI->uri->segment(2) == 'departmentlist')
        {
            if($active)
            {
                $str.='<li class="active">'.anchor('frontend/departmentlist/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
            else
            {
                $str.='<li>'.anchor('frontend/departmentlist/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
        }
        elseif($CI->uri->segment(2) == 'services')
        {
            if($active)
            {
                $str.='<li class="active">'.anchor('frontend/services/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
            else
            {
                $str.='<li>'.anchor('frontend/services/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
        }
        elseif($CI->uri->segment(2) == 'doctor')
        {
            if($active)
            {
                $str.='<li class="active">'.anchor('frontend/doctor/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
            else
            {
                $str.='<li>'.anchor('frontend/doctor/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
        }
        elseif($CI->uri->segment(2) == 'doctorslist')
        {
            if($active)
            {
                $str.='<li class="active">'.anchor('frontend/doctorslist/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
            else
            {
                $str.='<li>'.anchor('frontend/doctorslist/'.$CI->uri->segment(3).'/'.$item['code'], $item['code']).'</li>';
            }
        }
        elseif(is_numeric($CI->uri->segment(2)))
        {
            if($active)
            {
                $str.='<li class="active '.$CI->uri->segment(2).' ">'.anchor($item['code'].'/'.$CI->uri->segment(2), $item['code']).'</li>';
            }
            else
            {
...
if ( ! function_exists('get_ol_team')){
    function get_ol_team($array, $child = FALSE)
    {
        $str = '';
        ...
        if (count($array)) {
            ...
            foreach ($array as $item) {
                ...
                $str .= '<div class="" alt="'.$item['id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['first_name'].' '.$item['last_name'].'&nbsp;&nbsp;&nbsp;&nbsp;<span class="pull-right">
                            <div class="btn-group btn-group-xs">
                              <a class="btn btn-xs btn-primary" href="'.site_url('admin/team/edit/'.$item['id']).'"><i class="icon-edit"></i></a>
                              <a onclick="return confirm(\''.lang('Are you sure?').'\')" class="btn btn-xs btn-danger delete" data-loading-text="'.lang('Loading...').'" href="'.site_url('admin/team/delete/'.$item['id']).'"><i class="icon-remove"></i></a>
                            </div></span></div>';
                
                // Do we have any children?
                if (isset($item['children']) && count($item['children'])) {
                    $str .= get_ol_team($item['children'], TRUE);
                }
...
if ( ! function_exists('get_ol_partner')){
    function get_ol_partner($array, $child = FALSE)
    {
        $str = '';
        ...
        if (count($array)) {
            ...
            foreach ($array as $item) {
                $str .= '<li id="list_' . $item['id'] .'">';
                //$str .= '<div class="" alt="'.$item['id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['title'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-warning">'.$item['template'].'</span><span class="pull-right">
                $str .= '<div class="" alt="'.$item['id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['title'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="pull-right">
                            <div class="btn-group btn-group-xs">
                              <a class="btn btn-xs btn-primary" href="'.site_url('admin/partner/edit/'.$item['id']).'"><i class="icon-edit"></i></a>
                              <a onclick="return confirm(\''.lang('Are you sure?').'\')" class="btn btn-xs btn-danger delete" data-loading-text="'.lang('Loading...').'" href="'.site_url('admin/partner/delete/'.$item['id']).'"><i class="icon-remove"></i></a>
                            </div></span></div>';
                
                // Do we have any children?
                if (isset($item['children']) && count($item['children'])) {
                    $str .= get_ol_partner($item['children'], TRUE);
                }
...
if ( ! function_exists('get_ol_pages3'))
{
    function get_ol_pages3($array, $child = FALSE)
    {
        $str = '';
        
        if (count($array)) {
            $str .= $child == FALSE ? '<ol class="sortable" id="page_sortable3">' : '<ol>';
...
if ( ! function_exists('get_ol_pages2'))
{
    function get_ol_pages2($array, $child = FALSE)
    {
        $str = '';
        
        if (count($array)) {
            $str .= $child == FALSE ? '<ol class="sortable" id="page_sortable2">' : '<ol>';
...
if ( ! function_exists('get_ol_projects')){
    function get_ol_projects($array, $child = FALSE)
    {
        $str = '';
        ...
        if (count($array)) {
            ...
            foreach ($array as $item) {
                ...
                $str .= '<div class="" alt="'.$item['id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['title'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="pull-right">
                            <div class="btn-group btn-group-xs">
                              <a class="btn btn-xs btn-primary" href="'.site_url('admin/project/edit/'.$item['id']).'"><i class="icon-edit"></i></a>
                              <a onclick="return confirm(\''.lang('Are you sure?').'\')" class="btn btn-xs btn-danger delete" data-loading-text="'.lang('Loading...').'" href="'.site_url('admin/project/delete/'.$item['id']).'"><i class="icon-remove"></i></a>
                            </div></span></div>';
                
                // Do we have any children?
                if (isset($item['children']) && count($item['children'])) {
                    $str .= get_ol_projects($item['children'], TRUE);
                }
...
if ( ! function_exists('get_ol_categories')){
    function get_ol_categories($array, $child = FALSE)
    {
        $str = '';
            ...
            foreach ($array as $item) {
                ...
                $str .= '<div class="" alt="'.$item['id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['title'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="pull-right">
                            <div class="btn-group btn-group-xs">
                              ...
                              <a onclick="return confirm(\''.lang('Are you sure?').'\')" class="btn btn-xs btn-danger delete" data-loading-text="'.lang('Loading...').'" href="'.site_url('admin/category/delete/'.$item['id']).'"><i class="icon-remove"></i></a>
                            </div></span></div>';
                
                // Do we have any children?
                if (isset($item['children']) && count($item['children'])) {
                    $str .= get_ol_categories($item['children'], TRUE);
                }
    ...
    function get_ol_news($array, $child = FALSE)
    {
        ...
        if (count($array)) {
            ...
            foreach ($array as $item) {
                ...
                $str .= '<div class="" alt="'.$item['id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['title'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-warning">'.$item['template'].'</span><span class="pull-right">
                            <div class="btn-group btn-group-xs">
                              <a class="btn btn-xs btn-primary" href="'.site_url('admin/news/edit/'.$item['id']).'"><i class="icon-edit"></i></a>
...
{
    function get_all_services($array, $child = FALSE)
    {
       
        $str = '';
        
        if (count($array)) {
            $str .= $child == FALSE ? '<ol class="sortable" id="services_sortable">' : '<ol>';
            
            foreach ($array as $item) {
                $str .= '<li id="list_' . $item['services_id'] .'">';
                $str .= '<div class="" alt="'.$item['services_id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['name'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-warning">'.$item['template'].'</span><span class="pull-right">
                            <div class="btn-group btn-group-xs">
                              <a class="btn btn-xs btn-primary" href="'.site_url('admin/services/edit/'.$item['services_id']).'"><i class="icon-edit"></i></a>
                              <a onclick="return confirm(\''.lang('Are you sure?').'\')" class="btn btn-xs btn-danger delete" data-loading-text="'.lang('Loading...').'" href="'.site_url('admin/services/delete/'.$item['services_id']).'"><i class="icon-remove"></i></a>
                            </div></span></div>';
...
{
    function get_all_dept($array, $child = FALSE)
    {
        $str = '';
        
        if (count($array)) {
            $str .= $child == FALSE ? '<ol class="sortable" id="departments_sortable">' : '<ol>';
            
            foreach ($array as $item) {
                $str .= '<li id="list_' . $item['departments_id'] .'">';
                $str .= '<div class="" alt="'.$item['departments_id'].'" ><i class="icon-file-alt"></i>&nbsp;&nbsp;' . $item['name'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-warning">'.$item['template'].'</span><span class="pull-right">
                            <div class="btn-group btn-group-xs">
                              <a class="btn btn-xs btn-primary" href="'.site_url('admin/departments/edit/'.$item['departments_id']).'"><i class="icon-edit"></i></a>
                              <a onclick="return confirm(\''.lang('Are you sure?').'\')" class="btn btn-xs btn-danger delete" data-loading-text="'.lang('Loading...').'" href="'.site_url('admin/departments/delete/'.$item['departments_id']).'"><i class="icon-remove"></i></a>
                            </div></span></div>';
...
function cms_page_static($id,$lang_id){

    $CI =& get_instance();

    $CI->load->model('page_m');

   return $CI->page_m->get_page_static($id,$lang_id);

    
}

function actionSkypeStatus($username)
{
   if(isset($username))
   {
      $ostatus = fopen('http://mystatus.skype.com/'
      .intval(strip_tags($username)) . '.num', 'r');
