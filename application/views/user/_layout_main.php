...
<?php // $this->load->view('user/includes/chat_list'); ?>
...
© All rights reserved <?=print_value('users',array('id'=>$user_details->parent_id),'company_name');?>
...
if($checkDealerStatus){
	if($checkDealerStatus->status==0){
		...
		if($uri_string!=$lang_code.'/member/inactive')
			redirect($lang_code.'/member/inactive');
	}
}
else{
