...
if($user_details->play_w_pay==0){
	...
	if($camera_ids){
		...
		$all_datas = $this->comman_model->get_by('camera',array('user_id'=>$user_details->id,'is_expire'=>0,'payment_id !='=>0),false,false,false);
		if($all_datas){
...
<?php
	...
	if($camera_ids){
		$this->db->order_by('order','asc');
		$this->db->where_in('id',$camera_ids);
		$all_datas = $this->comman_model->get_by('camera',array('user_id'=>$user_details->id,'payment_id !='=>0,'is_expire'=>0),false,false,false);
		if($all_datas){
			foreach($all_datas  as $view_data){
?>	
<div class="col-md-4 " style="margin-bottom:10px;">
