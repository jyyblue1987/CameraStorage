...
<?php
$DD = $this->comman_model->get_by('users',array('id'=>$user_details->parent_id),false,false,true);
$dSupport = $this->comman_model->get_by('users_support',array('user_id'=>$user_details->parent_id),false,false,true);
$wLink = 'javascript:';
...
if($DD){
	if(!empty($DD->logo)){
?>
<a href="<?=$dSupport?$dSupport->website:'javascript:'?>" class="hidden-ss"  target="_blank"><img src="<?='assets/uploads/users/thumbnails/'.$DD->logo?>" style="width:100px;height:53px" /></a>
<?php
	}
?>
<!--<span style="font-weight:600;font-size:15px;"><?=$DD->company_name.', '.$DD->email.', '.$DD->phone;?></span>-->
<?php
}
?>
				</div>
...
if($DD){
	if(!empty($DD->logo)){
?>
<a href="<?=$dSupport?$dSupport->website:'javascript:'?>" class="hidden-lg hidden-md"  target="_blank"><img src="<?='assets/uploads/users/thumbnails/'.$DD->logo?>" style="width:100px;height:53px" /></a>
<?php
	}
?>
<!--<span style="font-weight:600;font-size:15px;"><?=$DD->company_name.', '.$DD->email.', '.$DD->phone;?></span>-->
