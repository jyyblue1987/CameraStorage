...
<div class="form-group" >
    ...
    <div class="col-lg-10">
		<select name="plan_id" class="form-control" id="input-plan-id" onchange="change_plan()" required >
        	<option value="">Select</option>
<?php
if(!empty($plan_data)){
	foreach($plan_data as $set_d){
?>
    <option value="<?=$set_d->id; ?>" <?=isset($products)&&$products->plan_id==$set_d->id?'selected="selected"':''?> price="<?=$set_d->price2;?>"  ><?=print_value('plans',array('id'=>$set_d->plan_id),'name').' ( '.$set_d->name.' $'.$set_d->price.'/mo USD (';?>
...
<div class="form-group" >
    ...
    <div class="col-lg-10">
		<?=form_input('price', set_value('price',$products->price), 'class="form-control " id="input-user-price" onkeyup="change_plan()" onkeydown="change_plan();" onkeypress="change_plan();" required')?>
...
<div class="form-group" >
    <label class="col-lg-2 control-label" style="padding-top:0">&nbsp;</label>
    <div class="col-lg-10">
		<div id="error-msge" style=""></div>
    </div>
</div>
...
<script>
//change_plan();
function change_plan(vals){
	val = $('#input-plan-id option:selected').attr('price');
	get_user_price = $('#input-user-price').val();
	if(val!=''&&get_user_price!=''){
		sVal = parseFloat(val);
		gUVal = parseFloat(get_user_price);
		if(gUVal<sVal){
			$('#error-msge').html('<input type="checkbox" name="set_conrifm"  value="1" required> <span style="color:#F00">Price is lower than your cost. Are you sure you want to do this?</span>');
		}
		else if(gUVal==sVal){
			$('#error-msge').html('<input type="checkbox" name="set_conrifm"  value="1" required> <span style="color:#F00">Price is same as your cost. Are you sure you want to do this?</span>');
		}
		else{
			$('#error-msge').html('');
		}
		//$('.default_price').html('$'+val);
	}
	else{
		$('#error-msge').html('');
	}
