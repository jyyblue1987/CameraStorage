...
<?php
echo get_ol1($permissions,$users,$user_details);

function get_ol1 ($array,$user_id,$user_details, $child = FALSE){
	$CI =& get_instance();
	$str = '';
	if (count($array)) {
...
//			echo $key.' : '.$item;
			$str .='<li>';
			$pos = checkPermission('users_permission',array('user_id'=>$user_id->id,'owner_id'=>$user_details->id,'type'=>$item['id'],'value'=>1));
//			echo '<br>'.$this->db->last_query();
			...
			if (isset($item['children']) && count($item['children'])) {
				$str .= '<ul class="checkbox2">'.get_ol1($item['children'],$user_id,$user_details, TRUE).'</ul>';
			}
			$str .='</li>';
		}
	//	$str .= '</ol>' . PHP_EOL;
