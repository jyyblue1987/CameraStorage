...
<?php $this->load->view('staff/includes/header_menu'); ?>
		
<?php $this->load->view('staff/includes/left_menu'); ?>
<?php // $this->load->view('staff/includes/chat_list'); ?>
		<div class="sidebar-bg"></div>
		<!-- end #sidebar -->
		
		<!-- begin #content -->
		<div id="content" class="content">
<?php $this->load->view('staff/includes/address'); ?>
...
<?php
$dealPop = $this->comman_model->get_by('users',array('id'=>$user_details->parent_id),false,false,true);
if($dealPop){
if(!empty($dealPop->company_name)){
?>

                    <?=site_url($lang_code.'/l/'.url_title($dealPop->company_name, 'dash', true).'/'.$dealPop->id)?>
...
<a href="http://www.facebook.com/sharer/sharer.php?u=<?=site_url($lang_code.'/l/'.url_title($dealPop->company_name, 'dash', true).'/'.$dealPop->id)?>" target="_blank" class="btn btn-social-icon btn-facebook">
    <i class="fa fa-facebook"></i>
</a>

<a href="https://plus.google.com/share?url=<?=site_url($lang_code.'/l/'.url_title($dealPop->company_name, 'dash', true).'/'.$dealPop->id)?>" target="_blank" class="btn btn-social-icon btn-google-plus">
    <i class="fa fa-google-plus"></i>
</a>

<a href="http://twitter.com/share?url=<?=site_url($lang_code.'/l/'.url_title($dealPop->company_name, 'dash', true).'/'.$dealPop->id)?>&text=Re" target="_blank" class="btn btn-social-icon btn-twitter">
    <i class="fa fa-twitter"></i>
</a>

<a href="http://www.linkedin.com/shareArticle?url=<?=site_url($lang_code.'/l/'.url_title($dealPop->company_name, 'dash', true).'/'.$dealPop->id)?>&title=Referrals Link&summary=<?=site_url($lang_code.'/l/'.url_title($dealPop->company_name, 'dash', true).'/'.$dealPop->id)?>" target="_blank" class="btn btn-social-icon btn-linkedin">
...
if($checkDealerStatus){
	if($checkDealerStatus->status==0){
		...
		if($uri_string!=$lang_code.'/staff/inactive')
			redirect($lang_code.'/staff/inactive');
	}
}
else{
	redirct($lang_code.'/staff/account/logout');
}
?>
