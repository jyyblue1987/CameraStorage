...
<tr>
    <td><?=$set_data->id; ?></td>
    <td><?=$set_data->name; ?></td>
    <td><?=print_value('plans',array('id'=>$set_data->plan_id),'name'); ?></td>
    <td>$<?=$set_data->price; ?></td>
    <td>$<?=$set_data->price2; ?></td>
    <td><?=array_key_exists($set_data->month,$month_data)?$month_data[$set_data->month]:$set_data->month?></td>
    <td>
    <a class="btn btn-icon-only btn-info" data-toggle="tooltip" data-placement="top"  title="Dealer Price Set"  href="<?=$_cancel.'/dealer_price/'.$set_data->id;?>">
