    ...
    <div class="table-responsive">
        ...
        <thead>
              <tr>
                ...
                <th><?=show_static_text($adminLangSession['lang_id'],2420);?>Employee</th>
<!--                <th class=""><?=show_static_text($adminLangSession['lang_id'],150003);?>Item</th>-->
                <th><?=show_static_text($adminLangSession['lang_id'],3017);?>Salary Amount</th>
                <th><?=show_static_text($adminLangSession['lang_id'],1503);?>Observations</th>
                <th><?=show_static_text($adminLangSession['lang_id'],1503);?>Account</th>
...
if(count($all_data)){
	...
	foreach($all_data as $set_data){
		$i++;
		$productName ='-';
		if($set_data->emp_id!=0){
			$product  = $this->comman_model->get_by('admin',array('id'=>$set_data->emp_id),false,false,true);
			if($product){
				$productName =$product->username;
			}
		}
		else{
			$productName =$set_data->username;
		}
