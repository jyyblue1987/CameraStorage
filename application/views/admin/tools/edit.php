...
<!--<div class="form-group">
  ...
  <div class="col-lg-10">
    ...
    <select class="form-control" name="style_id" id="select_category"  required>
        <option value="" >Select</option>
<?php
if(isset($styles_data)&&!empty($styles_data)){
	foreach($styles_data as $setCategory){
?>
<option value="<?=$setCategory->id;?>"  <?=($setCategory->id==$tags->style_id)?'selected="selected"':'';?> ><?=$setCategory->title;?></option>
<?php
	}
}
?>
        ...
        <?php echo form_dropdown('type', $type_data, $this->input->post('type') ? $this->input->post('type') : $tags->type, 'class="form-control" '); ?>
                           ...
                           <div style="margin-bottom: 0px;" class="tabbable">
                              ...
                              //  foreach($this->page_m->languages as $key_lang=>$val_lang):
                                foreach($this->tool_model->languages_icon as $key_lang=>$val_lang):
                              ...
                              <div style="padding-top: 9px; border-bottom: 1px solid #ddd;" class="tab-content">
                                <?php $i=0;foreach($this->tool_model->languages as $key_lang=>$val_lang):$i++;?>
                                <div id="<?=$key_lang?>" class="tab-pane <?=$i==1?'active':''?>">
                                    ...
                                    <div class="form-group">
                                      ...
                                      <div class="col-lg-10">
                                        <?=form_textarea('body_'.$key_lang, set_value('body_'.$key_lang, $tags->{'body_'.$key_lang}), 'placeholder="Body" rows="3" class="cleditor2 form-control"')?>
                                      </div>
                                    </div>
                                </div>
                                <?php endforeach;?>
