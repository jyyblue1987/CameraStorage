...
$ticketName = '-';
$userName = '-';
$userData = $this->comman_model->get_by('users',array('id'=>$view_data->user_id),false,false,true);
if($userData){
	$userName = $userData->first_name.' '.$userData->last_name;
}

if($view_data->ticket_id!=0){
	$storeData = $this->comman_model->get_by('tickets',array('id'=>$view_data->ticket_id),false,false,true);
	if($storeData){
		$ticketName = $storeData->name;
	}
}	

?>
    <div class="row static-info">
        <div class="col-md-3 name"><b>Username</b></div>
        <div class="col-md-9 value"><?=$userName;?></div>
    </div>    
    <div class="row static-info">
...
<div class="form-group" >
    ...
    <label class="col-lg-4 control-label">Change Status</label>
    <div class="col-lg-5">
...
<!--            <option value="Cancelled" <?=$view_data->status=='Cancelled'?'selected="selected"':'';?> >Cancelled</option>-->
            <option value="Completed" <?=$view_data->status=='Completed'?'selected="selected"':'';?> >Completed</option>
            <option value="In Progress" <?=$view_data->status=='In Progress'?'selected="selected"':'';?> >In Progress</option>
        </select>
    </div>
    <div class="col-lg-3">
            <?=form_submit('submit', show_static_text($adminLangSession['lang_id'],235), 'class="btn btn-primary"')?></div>
    </form>
</div>
<?php
}
else if($view_data->status =='Completed'){}
?>
                        </div>
                    </div>
...
?>
                ...
                <div class="form-group">
                  <label class="col-lg-2 control-label"><?=show_static_text($adminLangSession['lang_id'],2706);?>Comment</label>
                  <div class="col-lg-10">
        ...
        <div class="col-lg-10">
            <div id="fileuploader" class="fileuploader" style="background-color:#268abe"><i class="fa fa-plus"></i> <?=show_static_text($adminLangSession['lang_id'],278);?></div>
...
$( document ).ready(function() {
	$( "#status-form" ).submit(function() {
		...
		$('.ajax-error').show();
		var order_id ='<?=$view_data->id?>';
		var order_status= $('#order_status').val();
		$.ajax({
				type:"POST",
				url:"<?=$_cancel?>/ajax_save_status",
				data:{id:order_id,status:order_status,<?=$this->security->get_csrf_token_name();?>:'<?=$this->security->get_csrf_hash();?>'},
				dataType: 'json',
	...
	$( "#reply-form" ).submit(function() {
		...
		$.ajax({
				type:"POST",
				url:"<?=$_cancel?>/ajax_save_reply",
				data:{id:order_id,comment:comment,<?=$this->security->get_csrf_token_name();?>:'<?=$this->security->get_csrf_hash();?>'},
				dataType: 'json',
				success: function(json) {	
					if(json.result=='ok'){
						html = '<li><div class="timeline-icon">	<a href="javascript:;"><i class="fa fa-user"></i></a></div>'
								+'<div class="timeline-body"><div class="timeline-header">'
								+'<span class="username"><a href="javascript:;">'+username+'</a> <small></small></span>'
			                	+'<span class="pull-right text-muted">'+json.times+'</span></div>'
					            +'<div class="timeline-content"><p>'+comment+'</p></div></div></li>';
						//$('.timeline').append(html);
						$('.timeline li:last').prev().after(html);
					}
					if(json.status=='error'){
					}
				}
