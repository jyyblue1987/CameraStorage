...
<div class="form-group" >
        <label class="col-lg-2 control-label"><?=show_static_text($adminLangSession['lang_id'],4010);?>FPS</label>
        <div class="col-lg-10">

<select class="form-control " name="fps" id="select_category" required>
                    <option value="">Select</option>
<?php
$post_fps = $this->input->post('fps');
for($i=1;$i<=24;$i++){
	if($post_fps){
?>
    <option value="<?=$i?>" <?=$post_fps==$i?'selected="selected"':''?> ><?=$i.' fps'?></option>
<?php
	}
	else{
?>
    <option value="<?=$i?>" <?=$products->fps==$i?'selected="selected"':''?> ><?=$i.' fps'?></option>
<?php
	}
