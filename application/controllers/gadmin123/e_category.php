...
class E_category extends Admin_Controller
{
	public $_table_names = 'e_categories';
	public $_subView = 'admin/e_categories/';
	public $_redirect = '/e_category';
	public function __construct(){
		...
		$this->checkPermissions('expense_category');
    ...
    public function index()
	{
		...
		$this->data['all_data'] = $this->e_category_model->get_lang();
/*		$count = count($this->data['all_data']);
    ...
    public function search_ajax(){
        ...
        if (isset($_POST['sortable'])) {
            $this->e_category_model->save_order($_POST['sortable']);
        }
        
        // Fetch all pages
		$this->db->like('title',$title);
        $this->data['pages'] = $this->e_category_model->get_search_data($this->data['content_language_id']);
    ...
    public function order_ajax(){
        ...
        // Fetch all pages
        $this->data['pages'] = $this->e_category_model->get_nested($this->data['content_language_id']);
    ...
    public function edit($id = NULL)
	{
	    ...
	    if($id)
        {
            $this->data['categories'] = $this->e_category_model->get_lang($id, FALSE, $this->data['content_language_id']);
        ...
        else
        {
			...
			$this->data['title'] = $this->data['name'].' | '.$this->data['settings']['site_name'];
            $this->data['categories'] = $this->e_category_model->get_new();
        }
        

       
		// Pages for dropdown
        $this->data['pages_no_parents'] = $this->e_category_model->get_no_parents($this->data['content_language_id']);
        $this->data['page_languages'] = $this->language_model->get_form_dropdown('language');
        $this->data['templates_page'] = $this->e_category_model->get_templates('page_');
        // Fetch all files by repository_id
        
        // Set up the form
		$this->form_validation->set_message('required', '%s '.show_static_text($this->data['adminLangSession']['lang_id'],219));

        $rules = $this->e_category_model->rules;
        $this->form_validation->set_rules($this->e_category_model->get_all_rules());

        // Process the form
        if($this->form_validation->run() == TRUE)
        {
            $data =array();
            $data = $this->e_category_model->array_from_post(array('slug','parent_id'));
            if($id == NULL)$data['order'] = $this->e_category_model->max_order()+1;
            $data_lang = $this->e_category_model->array_from_post($this->e_category_model->get_lang_post_fields());
            if($id == NULL){
            ...
            $id = $this->e_category_model->save_with_lang($data, $data_lang, $id);
...
*/
    ...
    public function delete($id){
		...
		$this->e_category_model->delete($id);
        redirect($this->data['_cancel']);
	}
    ...
    public function _unique_slug($str)
    {
        ...
        $categories = $this->e_category_model->get();
