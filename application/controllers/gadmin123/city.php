...
{
	public $_table_names = 'cities';
	public $_subView = 'admin/cities/';
	public $_redirect = '/city';
	...
	{
	    // Fetch all pages
        $this->data['name'] = show_static_text($this->data['adminLangSession']['lang_id'],1670).'City';
        $this->data['title'] = $this->data['name'].' | '.$this->data['settings']['site_name'];
		//$this->data['all_data'] = $this->city_model->get_lang();
/*		$count = count($this->data['all_data']);
    ...
    public function edit($id = NULL){
        ...
        else
        {
			...
			$this->data['title'] = $this->data['name'].' | '.$this->data['settings']['site_name'];
            $this->data['categories'] = $this->city_model->get_new();
        }
        

		$this->data['country_data'] = $this->comman_model->get('regions',false);
		// Pages for dropdown
        ...
        $rules = $this->city_model->rules;
        $this->form_validation->set_rules($this->city_model->get_all_rules());

        // Process the form
        if($this->form_validation->run() == TRUE)
        {
            $data =array();
            $data = $this->comman_model->array_from_post(array('name','region_id'));

			$data['country'] = print_value('regions',array('id'=>$data['region_id']),'country');

            if($id == NULL)$data['order'] = $this->city_model->max_order()+1;
            $data_lang = $this->city_model->array_from_post($this->city_model->get_lang_post_fields());
            if($id == NULL){
                $data['created'] = time();
                $data['modified'] = time();
			}
...
*/
    ...
    public function order_ajax(){
        ...
        if (isset($_POST['sortable'])) {
            $this->city_model->save_order($_POST['sortable']);
        }
        
        // Fetch all pages
        //$this->data['pages'] = $this->city_model->get_nested($this->data['content_language_id']);
		$this->data['pages'] = $this->city_model->get_nested($this->data['content_language_id']);
