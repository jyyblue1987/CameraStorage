...
class Paypal_account extends Frontend_Controller{	
	public $_redirect = '/dealer/paypal_account';

	public $_subView = 'ownner/paypals/';
...
//		$this->_checkPaidUser();
	...
	function index(){
		...
		if($this->data['user_details']->hide_paypal_data==0){
			$this->data['all_data'] = $this->comman_model->get_by($this->_table_names,array('user_id'=>$this->data['user_details']->id,'type'=>'paypal'),false,false,false);
		}
    ...
    public function edit($id = NULL){
	    ...
	    if($id){
			...
			$checkProduct = $this->comman_model->get_by($this->_table_names,array('id'=>$id,'user_id'=>$this->data['user_details']->id,'type'=>'paypal'),false,false,true);
			if(!$checkProduct){
...
//		        'comission' => array('field'=>'comission', 'label'=>'Comission', 'rules'=>'trim|required|integer'),
        ...
        if($this->form_validation->run() == TRUE){
            $data 	= array();
			$post1 =array('username','signature','password','is_default','comission','account','is_recurring');
	...
	public function unhide_form(){
        ...
        // Process the form
        if($this->form_validation->run() == TRUE){
			$post1 =array('password');
        	$data = $this->comman_model->array_from_post($post1);
//			echo $this->data['user_details']->password.' == '.$data['password'];die;
			if($this->data['user_details']->password == $data['password']){
				$this->db->where('id',$this->data['user_details']->id);
				$this->db->set(array('hide_paypal_data'=>0));
				$this->db->update('users');
				redirect($this->data['_cancel']);
			}
			else{
			...
			//redirect($this->data['_cancel']);
			redirect($this->data['_cancel'].'/unhide_form');
        }

		$this->data['name'] = 'Paypal API';
		$this->data['title'] = $this->data['name'].' | '.$this->data['settings']['site_name'];
        $this->data['subview'] = $this->_subView.'password';
		$this->load->view($this->_mainView,$this->data);
	}

	function set_hide(){
		$check_product = $this->comman_model->save('users',array('hide_paypal_data'=>1),$this->data['user_details']->id);
		redirect($this->data['_cancel']);			
	}
