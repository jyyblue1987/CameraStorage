...
/*		echo '<br>';
		...
		if($all_user){
			...
			//for send notification for update balance
			foreach($all_user as $set_u){
				$checkTrue =false;
				if($set_u->total_point<0){//if total_point <0
					if(empty($set_u->disable_date)){
						$this->db->where('id', $set_u->id);
						$this->db->set('disable_date',date('Y-m-d'),true);
						$this->db->update('users');
					}
					else{
						if($set_u->status==1){
							$DDS = strtotime('+15 day', strtotime($set_u->disable_date));
							if($DDS<$now){
							//	echo 'asd';
								$this->db->where('id', $set_u->id);
					...
					if(!empty($set_u->is_due_date)){
						$uDate = strtotime($set_u->is_due_date);
						if($uDate<$now){
							$checkTrue =true;
						}
					}
					...
					if($checkTrue){
						$email_data = $this->comman_model->get_by('email',array('id'=>6),false,false,true);
						...
						$email_data->message = str_replace('{user_name}', $set_u->username, $email_data->message);
						$email_data->message = str_replace('{amount}', '$'.abs($set_u->total_point), $email_data->message);
						...
						$email_data->message = str_replace('{update_link}', base_url().$this->data['lang_code'].'/dealer/update_balance', $email_data->message);
						...
						$this->email->to($set_u->email);
						$this->email->subject($email_data->subject);
						$this->email->message($email_data->message);
						$this->email->send();
						
						$this->db->where('id', $set_u->id);
						$this->db->set('is_due_date',date('Y-m-d'),true);
						$this->db->update('users');
					}
					//echo '<br>'.$set_u->id;
				}
