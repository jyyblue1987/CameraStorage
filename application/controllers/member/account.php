	...
	public function __construct(){
		...
		if(isset($detail['user_session'])){	
			...
			if(!empty($this->data['user_account'])){
				...
				if($this->data['user_account']->status!=1){
					$this->logout();
				}
			}
			else{
...
//		$this->data['country_data'] = $this->custom_model->get_country_name();
        ...
        if($this->form_validation->run() == TRUE){
			$post_data = $this->comman_model->array_from_post(array('first_name','company_name','address','address2','state','city','phone','phone2','zip'));
...
/*				echo '<br><br>ExpireD: '.date('Y-m-d',$expireData);
		...
		$checkProductss = $this->comman_model->get_by('camera',array('user_id'=>$this->data['user_details']->id,'sys_file'=>0,'is_connect'=>0,'is_expire'=>0,'payment_id !='=>0),false,false,false);
		$output['connect'] = count($checkProductss);
