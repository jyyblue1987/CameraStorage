...
//			echo $this->db->last_query();die;
			...
			if($checkUserApi){
				if($checkUserApi->username!=''&&$checkUserApi->signature!=''&&$checkUserApi->password!=''){
					...
					$userAPIArr['sandbox']		= $checkUserApi->sandbox;
			...
			$per_day_amount = round($checkcamera->price/$totalDay,2);
...
/*			echo '<br>'.$per_day_amount;
			echo '<br>'.$per_day_amount;
			echo '<br>'.$totalAmount;die;*/
/*				echo $totalAmount;
			die;*/
//			$str .= "&L_PAYMENTREQUEST_0_AMT0=".$totalAmount."&L_PAYMENTREQUEST_0_NAME0=".$checkMembership->name." Total amount &L_PAYMENTREQUEST_0_QTY0=1";
			...
			if($resArray){
				...
				if($ack=="SUCCESS" || $ack=="SUCCESSWITHWARNING"){
					...
					'api_sandbox'		=> $userAPIArr['sandbox'],
...
/*		echo 'ehco '.$token;
		...
		if ($token!=""){
			$userAPI =array(
						...
						'sandbox'=>$this->data['session_data']['user_subscribe']['api_sandbox'],
						);
			...
			if( $ack == "SUCCESS" || $ack == "SUCESSWITHWARNING"){
				...
				if(isset($membership_paypal_sess)&&!empty($membership_paypal_sess)){
					...
					$post_data =array(
							...
							'month'				=> $this->data['session_data']['user_subscribe']['remember_day'], //30,//$this->data['session_data']['user_subscribe']['month'],
							);
		...
		//	die;
			...
			if( $ack == "SUCCESS" || $ack == "SUCCESSWITHWARNING" ){						
				...
				if($this->data['session_data']['user_subscribe']['recurring']==1){
					$post_data['month'] 		= 30;
					$post_data['on_date'] 		= date('Y-m-t');
					$post_data['on_datetime']	= date('Y-m-t H:i');
				}
				$this->db->trans_start();
				...
				if($this->data['session_data']['user_subscribe']['recurring']==1){
					$this->db->set('plan_day',$this->data['session_data']['user_subscribe']['remember_day'], true);
					$this->db->set('plan_date',date('Y-m-d'), true);
				}
				else{
					$this->db->set('plan_day',30, true);
					$this->db->set('plan_date',date('Y-m-t'), true);
				}
				$this->db->set('payment_type', 'paid', true);
				$this->db->set('is_test',0, true);
				$this->db->set('is_expire',0, true);
...
//					echo 'asd';
	...
	function recurring_method($payment_id){
		...
		$userAPI =array(
						...
						'sandbox'		=> $payment_data->api_sandbox,
						);

		$nextDate = date('Y').'-'.date('m',strtotime('+1 month')).'-01';
		$recurring_Arr = array(
							...
							'billing_period'		=> 'Month',// Month,Day
							's_date'				=> $nextDate.'T'.date('H:i:s').'Z',
							'billing_recurring'		=> 1,
						);
		...
		if($recurring_data['ACK']=='Success'||$recurring_data['ACK']=='success'){
			...
			$this->db->where('id',$payment_data->product_id);
			$this->db->set('is_recurring',1, true);
