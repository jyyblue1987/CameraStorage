...
//		$this->load->library('user_agent');		
	...
	/*					echo '<br>Old: '.$check->s_file;
						...
						//$string = 'ffmpeg -rtsp_transport tcp -i "'.$videoLink.'" -t 900 "/var/www/html/data/'.$path.'/'.$fileStart.'" > /dev/null 2>&1 &';
						$string = 'ffmpeg -nostats -loglevel 0 -rtsp_transport tcp -i "'.$videoLink.'" -t 10 "/var/www/html/data/'.$path.'/'.$fileStart.'" > /dev/null 2>&1 &';;
						//$string = 'ffmpeg -rtsp_transport tcp -i "'.$videoLink.'" -t 900 "/var/www/html/data/'.$path.'/'.$fileStart.'" > /dev/null 2>&1 &';
		...
		if($id){
			...
			if($check){
				...
				if($get_last_file){
					...
					$this->db->set('is_worked',0,true);
					$this->db->update('camera');
					$output['msge'] ='We are unable to connect to your camera. Please double-check your settings and try again.';
				}
			}
			else{
				$output['msge'] ='No camera found!!';
			}
