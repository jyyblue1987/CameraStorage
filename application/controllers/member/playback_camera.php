...
class Playback_camera extends Frontend_Controller{	
	...
	function index(){
		...
		$this->data['all_data'] = $this->comman_model->get_by($this->_table_names,array('user_id'=>$this->data['user_details']->id,'is_expire'=>0),false,array('order_playback'=>'asc'),false);
		//$this->load->view('user/orders',$this->data);
	...
	function send_custom(){
		...
		if($this->input->get('id')){
			...
			if($this->data['settings']['email_option']==1){
				...
				$string = 'SELECT users_e_setup.*,users.email AS u_email,users.company_name AS company_name FROM users_e_setup JOIN users ON users_e_setup.user_id = users.id WHERE users.id= '.$this->data['user_details']->parent_id;
...
/*				if(is_file($checkAlreayExist)){
	...
	function send_mail_ci($checkUser,$check,$subject,$body){
		...
		if($email_data){
			...
			if($support->type=='smtp'){
				...
				$this->email->initialize($config);
				$this->email->from($support->u_email, $support->company_name);
				$this->email->to($checkUser->email);
				$this->email->subject($subject);
				$this->email->message($body);
				$this->email->send();	
