...
//		$this->data['all_c_data2'] = count($this->comman_model->get_by($this->_table_names,array('user_id'=>$this->data['user_details']->id,'payment_id !='=>0,'model'=>'IP Camera','sys_file'=>0,'is_connect'=>0),false,false,false));
	...
	public function edit($id = NULL){
		...
		if($checkProduct->sys_file==1){
			$this->session->set_flashdata('error','Sorry, you cannot edit live/paid camera.'); 			
			redirect($this->data['_cancel']);
		}
        ...
        if($this->form_validation->run() == TRUE){
			//die;
            $data 	= array();
			$post1 =array('name','username','password','ip_address','port','channel','stream','camera_num','model','resolution','url','is_mail','timeszone');
...
//			$data['enabled'] 	= 0;
			...
			$files = $checkProduct->camera_path.'/camera '.$checkProduct->id;
			$image = '/var/www/html/data/'.$files;
			if(is_file($image))
    	    	unlink($image);
			//$this->session->set_flashdata('success','Data has successfully updated.'); 
///			redirect($this->data['_cancel']);
			redirect($this->data['_cancel'].'/test_camera/'.$id);
        }
    ...
    public function create(){
        ...
        if($this->form_validation->run() == TRUE){
			//die;
            $data 	= array();
			$post1 =array('name','username','password','ip_address','port','channel','stream','camera_num','model','m_id','resolution','url','is_mail','timeszone');
...
/*				echo 'Yes';		
	...
	function delete($id=false){
		...
		if(!$check_product){
			$this->session->set_flashdata('error','Sorry! you cannot delete.');
			redirect($this->data['_cancel']);			
		}
...
//		$new_files = 'data/camera '.$id;
	...
	function ajax_delete(){
		...
		$action =false;
		if(!$id){
			$output['message'] = 'There is no camera.';
		}
		$check_product = $this->comman_model->get_by($this->_table_names,array('id'=>$id,'user_id'=>$this->data['user_details']->id),FALSE,FALSE,TRUE);
		if(!$check_product){
			$output['message'] = 'There is no camera.';
		...
		if($action){
			...
			$output['message'] = '';
			$nowtime = time();
			$add_day_3 = strtotime('+3 day', $check_product->created);
/*			echo '<br>'.date('d-m-Y, H:i',$nowtime);
			echo '<br>'.date('d-m-Y, H:i',$check_product->created);
			die;*/
			$this->change_subscription($check_product->id);
			if($nowtime<$add_day_3){
				$check_refund = $this->refund_payment($check_product->id);
				if($check_refund['status']=='ok'){
					$this->db->where('id',$this->data['user_details']->parent_id);
					$this->db->set('total_point', 'ROUND(total_point+'.abs($check_product->dealer_price).',2)', FALSE);
...
/*			if(ENVIRONMENT=='production'){
			...
			$this->load->helper("file");
			h_deleteDirectory($new_files.'/');*/
			$check_camera_delete = $this->comman_model->get_by($this->_table_names.'_deleted',array('camera_id'=>$id),FALSE,FALSE,TRUE);
	...
	function change_subscription($camera_id=false){
        $this->load->model('recurring_model');
		$result = $this->recurring_model->cancel_recurring($camera_id);
		return $result;
	}	

	function cancel_payment($camera_id=false){//for testing
        $this->load->model('recurring_model');
		$result = $this->recurring_model->cancel_recurring_payment($camera_id);
	//	printR($result);
		return $result;
	}	
	

	function refund_payment($camera_id=false){
		$result = array('status'=>'error');
		if($camera_id){
	        $this->load->model('recurring_model');
			$result = $this->recurring_model->refund_recurring_payment($camera_id);
		}
//		printR($result);
		return $result;
	}	
