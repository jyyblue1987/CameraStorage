...
class Camera extends Frontend_Controller{	
	...
	function index(){
		...
		$this->data['all_data'] = $this->comman_model->get_by($this->_table_names,array('user_id'=>$this->data['user_details']->parent_id,'model'=>'NVR'),false,false,false);
		$this->data['all_data2'] = $this->comman_model->get_by($this->_table_names,array('user_id'=>$this->data['user_details']->parent_id,'model'=>'IP Camera'),false,false,false);
		$this->data['all_data3'] = $this->comman_model->get_by($this->_table_names,array('user_id'=>$this->data['user_details']->parent_id,'model'=>'Custom RTSP Device'),false,false,false);
	...
	//	$id = 5;
		if($id){
			...
			if($check){
				...
				elseif($check->model=='IP Camera'){
					$videoLink = 'rtsp://'.$check->ip_address.':'.$check->port.'/user='.$check->username.'&password='.$check->password.'&channel='.$check->camera_num.'&stream='.$check->stream.'.sdp';
				}
				elseif($check->model=='Custom RTSP Device'){
					$videoLink = $check->url;
				}
				else{
					$videoLink = 'rtsp://demo.iTechProducts.info:'.$check->port.'/video'.$check->stream.'.sdp';
				}
