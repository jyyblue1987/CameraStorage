...
class Playback_camera extends Frontend_Controller{	
	public $_redirect = '/user/playback_camera';

	public $_subView = 'employee/playback_camera/';
...
//		$this->_checkPaidUser();
	...
	function index(){
		...
		$this->data['all_data'] = $this->comman_model->get_by($this->_table_names,array('user_id'=>$this->data['userClient']->id,'payment_id !='=>0,'is_expire'=>0),false,array('order_playback'=>'asc'),false);
	...
	public function v($id = false){
		...
		$this->data['view_data'] = $checkProduct = $this->comman_model->get_by($this->_table_names,array('id'=>$id,'user_id'=>$this->data['userClient']->id),false,false,true);
		if(!$checkProduct){
			redirect($this->data['_cancel']);
		}

		if($this->data['userClient']->play_w_pay==1){
		...
		$enable_date = $this->data['disabled_date'] = h_get_playback_date(date('Y-m-d'),$checkProduct->m_day,'m-d-Y');
		$startDate = $endDate = '';
...
/*		$images_folder = 'camera '.$id;
		...
		$images_folder = 'camera '.$id;
		$user_path = $this->data['userClient']->dealer_path.'/'.$this->data['userClient']->folder_path;
...
//			echo 'data/'.$checkProduct->camera_path.'/'.$images_folder;
		...
		//for localhost
		$u_b_path = $this->data['userClient']->dealer_path.'\\'.$this->data['userClient']->folder_path;
...
//		echo 'Yes';
		...
		if($this->data['settings']['email_option']==1){
			$checkEmail = $this->comman_model->get_by('users_email',array('user_id'=>$this->data['userClient']->parent_id,'email_id'=>6),false,false,true);
			if($checkEmail){
				$html = $checkEmail->message;
				$subject = $checkEmail->subject;
			}
			else{
				$this->session->set_flashdata('error','Sorry!! Problem sending this video clip via E-Mail!!'); 
				redirect($this->data['_cancel'].'/v/'.$id);
			}

			$string = 'SELECT users_e_setup.*,users.email AS u_email,users.company_name AS company_name FROM users_e_setup JOIN users ON users_e_setup.user_id = users.id WHERE users.id= '.$this->data['userClient']->parent_id;
			...
			$html= str_replace('{user_name}', $this->data['userClient']->username, $html);
			$html= str_replace('{camera_name}', $checkProduct->name, $html);
			$html= str_replace('{download_link}','<a href="'.site_url('video_download/download?id='.$checkProduct->id.'&files='.$file).'">Download</a>', $html);
//			echo $html;die;
			//check send mail
			$sentMail = false;
...
/*				if(is_file($checkAlreayExist)){
	...
	//			echo 'Yes';
				$this->session->set_flashdata('success','Video clip has successfully sent!!'); 
			}
			else{
				//$print =  $this->email->print_debugger();
				//printR($print);
				
				$this->session->set_flashdata('error','Problem sending this video clip via E-Mail!!'); 
			}
	...
	function send_custom(){
		...
		if($this->input->get('id')){
			...
			if($this->data['settings']['email_option']==1){
				...
				$html= str_replace('{camera_name}', $checkProduct->name, $html);
				$html .= '<br>Download Link: <a href="'.site_url('video_download/download?id='.$checkProduct->id.'&files='.$file).'">Download</a>';
...
/*					if(is_file($checkAlreayExist)){
				...
				$this->session->set_flashdata('error','Sorry!! Video clip can not be send!!'); 
				redirect($this->data['_cancel'].'/v/'.$id);
			}			
