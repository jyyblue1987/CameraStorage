			...
			foreach($all_data as $set_data){
				...
				if($cameraPlay){
					...
					if(ENVIRONMENT=='production'){
						curl_setopt($ch, CURLOPT_URL, "http://localhost/myonlinecameras/ajax_camera/ajaxCameraRun?id=".$set_data->id);
					}
					else{
						curl_setopt($ch, CURLOPT_URL, "http://myonlinecameras.com/ajax_camera/ajaxCameraRun?id=".$set_data->id);
					}
...
//		printf("Elapsed time: %.2f seconds\n", microtime(true) - $start);
	...
	function ajaxCameraRun(){
		...
		//$id = 10;
		if($id){
			$check = $this->comman_model->get_by('camera',array('id'=>$id,'is_connect'=>0,'sys_file'=>0),false,false,true);
