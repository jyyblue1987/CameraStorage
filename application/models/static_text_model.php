...
class Static_text_model extends MY_Model {
    protected $_table_name = 'static_text';
...
*/        
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->join('news_lang','news.id = news_lang.static_text_id')
    ...
    public function get_id_by_name ($static_text_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.static_text_id');
		...
		foreach ($news as $n) {
		  if(url_title_cro($n['title'], '-', TRUE) == $static_text_id)  
          {
            return $n['id'];
          }
		}
