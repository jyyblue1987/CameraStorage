...
*/	
	...
	public function get_nested_playback($user_id=false,$limit=false){
        if($limit){
            $this->db->limit($limit);
        }
        $this->db->select('*');
        ...
        $this->db->order_by('order_playback','asc');
		$categories = $this->db->get()->result_array();
	...
	public function save_order_playback ($data)
	{
		if (count($data)) {
			foreach ($data as $order => $categories) {
				if ($categories['item_id'] != '') {
					$data = array('parent_id' => (int) $categories['parent_id'], 'order_playback' => $order);
					$this->db->set($data)->where($this->_primary_key, $categories['item_id'])->update($this->_table_name);
				}
			}
		}
