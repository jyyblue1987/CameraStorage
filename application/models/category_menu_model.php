...
class Category_menu_model extends MY_Model {
    ...
    protected $_table_name = 'page_menu';
...
/*        $templatesDirectory = opendir(APPPATH.'../templates/'.$CI->app_settings['template']);
	...
	public function get_lang($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.page_menu_id');
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->join('categories_lang','categories.id = categories_lang.page_menu_id')
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        // Save lang data
        $this->db->delete($this->_table_name.'_lang', array('page_menu_id' => $id));
        ...
        foreach($this->languages as $lang_key=>$lang_val)
        {
            if(is_numeric($lang_key))
            {
                ...
                $curr_data_lang['page_menu_id'] = $id;
    ...
    public function get_id_by_name ($page_menu_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.page_menu_id');
		...
		foreach ($categories as $n) {
		  if(url_title_cro($n['title'], '-', TRUE) == $page_menu_id)  
          {
            return $n['id'];
          }
		}
    ...
    {
        $this->db->delete('categories_lang', array('page_menu_id' => $id)); 
		...
		$this->db->set(array('page_menu_id' => 0))->where('page_menu_id', $id)->update('products');
		$this->db->set(array('sub_page_menu_id' => 0))->where('sub_page_menu_id', $id)->update('products');
    }
