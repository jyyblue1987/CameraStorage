...
class Quiz_model extends MY_Model {    
   ...
   public $rules_lang = array();
	...
	public function get_lang($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.quiz_id');
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->join('categories_lang','categories.id = categories_lang.quiz_id')
    ...
    public function save_category_product($data, $id){
		$this->db->delete('product_category', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				$this->db->insert('product_category',array('category_id'=>$value,'quiz_id'=>$id));
			}
		}
	}
    public function save_colour_product($data, $id){
		$this->db->delete('product_colour', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				$this->db->insert('product_colour',array('colour_id'=>$value,'quiz_id'=>$id));
			}
		}
	}

    public function save_size_product($data, $id){
		$this->db->delete('product_size', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				$this->db->insert('product_size',array('size_id'=>$value,'quiz_id'=>$id));
			}
		}
	}

    public function save_brand_product($data, $id){
		$this->db->delete('product_brand', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				$this->db->insert('product_brand',array('brand_id'=>$value,'quiz_id'=>$id));
			}
		}
	}
    ...
    public function save_tag_product($data, $id){
		$this->db->delete('product_tag', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				$this->db->insert('product_tag',array('tag_id'=>$value,'quiz_id'=>$id));
			}
		}
	}

    public function save_attribute_product($data, $id,$att_id){
		$this->db->delete('product_attribute', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				...
				if($check_value){
					...
					if($check_field){						
						$this->db->insert('product_attribute',array('value_id'=>$value,'quiz_id'=>$id,'attribute_id'=>$att_id,'field_id'=>$check_field->id,));
    ...
    public function save_location_product($data, $id){
		$this->db->delete('product_location', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				$this->db->insert('product_location',array('location_id'=>$value,'quiz_id'=>$id));
			}
		}
	}

    public function save_store_product($data, $id){
		$this->db->delete('product_store', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				$this->db->insert('product_store',array('store_id'=>$value,'quiz_id'=>$id));
			}
		}
	}
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        // Save lang data
        $this->db->delete($this->_table_name.'_lang', array('quiz_id' => $id));
    ...
    public function get_id_by_name ($quiz_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.quiz_id');
		...
		foreach ($categories as $n) {
		  if(url_title_cro($n['title'], '-', TRUE) == $quiz_id)  
          {
            return $n['id'];
          }
		}
...
/*		$this->db->delete('product_tag', array('quiz_id'=>$id));
		...
		//$this->db->set(array('quiz_id' => 0))->where('quiz_id', $id)->update('projects');
    }
    ...
    public function save_options($data,$price_data,$options_id, $id){
		...
		//$this->db->delete('posts_option', array('quiz_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				if(isset($options_id[$key])){
					...
					$this->db->insert('products_option',array('name'=>$value,'price'=>$price_data[$key],'quiz_id'=>$id));
				}
			}
		}
	}
		...
		if($data){
			foreach($data as $key=>$value){
				if(isset($options_id[$key])){
					...
					$this->db->insert('products_extra',array('name'=>$value,'price'=>$price_data[$key],'quiz_id'=>$id));
				}
			}
		}
	}
	...
	function get_product_options($quiz_id,$level=false){
