...
class Gallery_model extends MY_Model {    
   ...
   public $rules_lang = array();
	...
	public function get_lang($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.gallery_id');
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->join('categories_lang','categories.id = categories_lang.gallery_id')
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        // Save lang data
        $this->db->delete($this->_table_name.'_lang', array('gallery_id' => $id));
    ...
    public function get_id_by_name ($gallery_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.gallery_id');
		...
		foreach ($categories as $n) {
		  if(url_title_cro($n['title'], '-', TRUE) == $gallery_id)  
          {
            return $n['id'];
          }
		}
    ...
    public function delete($id)
    {
		...
		$this->db->delete($this->_table_name.'_flies', array('gallery_id'=>$id));
		// Reset parent ID for its children
		//$this->db->set(array('gallery_id' => 0))->where('gallery_id', $id)->update('projects');
    }
