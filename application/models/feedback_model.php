...
class feedback_model extends MY_Model {
    ...
    public $feedback_admin_rules = array(
        'user_name' => array('field'=>'user_name', 'label'=>'User Name', 'rules'=>'trim|required|max_length[100]'),
        'title' => array('field'=>'title', 'label'=>'Title', 'rules'=>'trim|required|max_length[100]'),
        'company_name' => array('field'=>'company_name', 'label'=>'Company Name', 'rules'=>'trim|required|max_length[100]'),
        'description' => array('field'=>'description', 'label'=>'description', 'rules'=>'trim|required'),
   );
...
/*            $this->rules_lang["title_$key"] = array('field'=>"title_$key", 'label'=>'lang:Title', 'rules'=>'trim|required|xss_clean');
    ...
    public function get_new_feedback_admin(){
        $news = new stdClass();
        $news->description = '';
        $news->title = '';
        $news->user_name = '';
    ...
    public function get_new()
	{
        ...
        {
            $news->{"title_$key"} = '';
            $news->{"navigation_title_$key"} = '';
            //$page->{"slug_$key"} = '';
            $news->{"body_$key"} = '';
            $news->{"keywords_$key"} = '';
            $news->{"description_$key"} = '';
        }
...
*/        
    ...
    public function get_lang($id = NULL, $single = FALSE, $lang_id=1){
        ...
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.blog_id');
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->join('news_lang','news.id = news_lang.blog_id')
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        // Save lang data
        $this->db->delete($this->_table_name.'_lang', array('blog_id' => $id));
    ...
    public function get_id_by_name ($blog_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.blog_id');
		...
		foreach ($news as $n) {
		  if(url_title_cro($n['title'], '-', TRUE) == $blog_id)  
          {
            return $n['id'];
          }
		}
    ...
    public function delete($id)
    {
        ...
        $this->db->delete('blogs_lang', array('blog_id' => $id));  
      //  $this->db->delete('blog_like', array('blog_id' => $id));  
