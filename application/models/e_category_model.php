...
class E_category_model extends MY_Model {
    
    protected $_table_name = 'e_categories';
...
/*        $templatesDirectory = opendir(APPPATH.'../templates/'.$CI->app_settings['template']);
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->where(array($this->_table_name.'_lang.language_id'=>$lang_id,$this->_table_name.'.id'=>$id))
                    ->join($this->_table_name.'_lang',$this->_table_name.'.id ='.$this->_table_name.'_lang.category_id')
	...
	public function get_nested($lang_id=1,$limit=false)
	{
        if($limit){
            $this->db->limit($limit);
        }
