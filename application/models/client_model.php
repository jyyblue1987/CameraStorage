...
class Client_model extends MY_Model {
    ...
    protected $_table_name = 'client_reviews';
    protected $_order_by = 'parent_id, order, id';
    public $rules = array(
        'page_id' => array('field'=>'page_id', 'label'=>'lang:Page', 'rules'=>'trim|intval'),
        'first_name' => array('field'=>'first_name', 'label'=>'lang:Name', 'rules'=>'trim|required|max_length[100]|xss_clean'),
   ...
   public $rules_lang = array();
    ...
    public function __construct(){
        ...
        {
            $this->rules_lang["body_$key"] = array('field'=>"body_$key", 'label'=>'Designation', 'rules'=>'trim|required|xss_clean');
            $this->rules_lang["description_$key"] = array('field'=>"description_$key", 'label'=>'Feedback', 'rules'=>'trim|required|xss_clean');
        }
    ...
    public function get_new()
    {
        ...
        {
            $team->{"title_$key"} = '';
            $team->{"navigation_title_$key"} = '';
            //$team->{"slug_$key"} = '';
            $team->{"body_$key"} = '';
            $team->{"keywords_$key"} = '';
            $team->{"description_$key"} = '';
        }
...
*/        
    ...
    public function get_lang_banner($lang_id=1){
        ...
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.client_id');
    ...
    function delete_lang($client_id){
		$this->db->delete($this->_table_name.'_lang', array('client_id' => $client_id));
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        else
        {
            ...
            $this->db->delete($this->_table_name.'_lang', array('client_id' => $id));
    ...
    public function get_id_by_name ($client_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.client_id');
        ...
        foreach ($pages as $page) {
          if(url_title_cro($page['title'], '-', TRUE) == $client_id)  
          {
            return $page['id'];
          }
        }
