...
class News_model extends MY_Model {
    ...
    public $email_rules = array(
        'parent_id' => array('field'=>'parent_id', 'label'=>'lang:Parent', 'rules'=>'trim|intval'),
        'desc' => array('field'=>'desc', 'label'=>'Message', 'rules'=>'trim|required'),
...
*/        
    ...
    public function get_lang($id = NULL, $single = FALSE, $lang_id=1){
        ...
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.news_id');
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->join('news_lang','news.id = news_lang.news_id')
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        // Save lang data
        $this->db->delete($this->_table_name.'_lang', array('news_id' => $id));
    ...
    public function get_id_by_name ($news_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.news_id');
		...
		foreach ($news as $n) {
		  if(url_title_cro($n['title'], '-', TRUE) == $news_id)  
          {
            return $n['id'];
          }
		}
    ...
    {
        $this->db->delete('news_lang', array('news_id' => $id));  
        // Remove repository
