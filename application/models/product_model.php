...
class Product_model extends MY_Model {    
    ...
    public $rules = array(
        ...
        'code'				=> array('field'=>'code', 'label'=>'Code', 'rules'=>'trim|required'),
        'minimum_order'		=> array('field'=>'minimum_order', 'label'=>'Minimum Order', 'rules'=>'trim|required'),
        'reorder_point'		=> array('field'=>'reorder_point', 'label'=>'Reorder Point', 'rules'=>'trim|required'),
//        'tax' 				=> array('field'=>'tax', 'label'=>'Tax', 'rules'=>'trim|integer'),
   );

    public $s_rules = array(
        'category_id'		=> array('field'=>'category_id', 'label'=>'Category', 'rules'=>'trim|intval'),
        'language_id' 		=> array('field'=>'language_id', 'label'=>'lang:Language', 'rules'=>'trim|intval'),
        'price' 			=> array('field'=>'price', 'label'=>'Price1', 'rules'=>'trim|required|numeric'),
        'vat' 				=> array('field'=>'vat', 'label'=>'Vat', 'rules'=>'trim|integer'),
        'price'				=> array('field'=>'price', 'label'=>'Price', 'rules'=>'trim|required|numeric'),
        'discount_price'	=> array('field'=>'discount_price', 'label'=>'Discount', 'rules'=>'trim|required|numeric'),
      //  'tax' => array('field'=>'tax', 'label'=>'Tax', 'rules'=>'trim|integer'),
   );

    public $buy_rules = array(
        'product_id'		=> array('field'=>'product_id', 'label'=>'Product', 'rules'=>'trim|intval'),
        'quantity' 			=> array('field'=>'quantity', 'label'=>'Quantity', 'rules'=>'trim|integer'),
        'price' 			=> array('field'=>'price', 'label'=>'Cost', 'rules'=>'trim|required|numeric'),
        'tax' 				=> array('field'=>'tax', 'label'=>'Tax', 'rules'=>'trim|integer'),
        'price'				=> array('field'=>'price', 'label'=>'Price', 'rules'=>'trim|required|numeric'),
        'dates' 			=> array('field'=>'dates', 'label'=>'dates', 'rules'=>'trim|required'),
      //  'tax' => array('field'=>'tax', 'label'=>'Tax', 'rules'=>'trim|integer'),
   );
   ...
   public $rules_lang = array();
    ...
    public function get_new_buy_order(){
...
/*    public function save_category_product($data, $id){
    ...
    public function save_category_product($data, $id){		
		$this->db->delete('products_field', array('product_id' => $id)); 
		if($data){
			foreach($data as $key=>$value){
				if($value)
					$this->db->insert('products_field',array('value_id'=>$value,'field_id'=>$key,'product_id'=>$id));
			}
		}
	}
			...
			foreach($data as $key=>$value){
				if($value){
					foreach($value as $key1=>$value1){
						$this->db->insert('products_field',array('value_id'=>$value1,'field_id'=>$key,'product_id'=>$id));
					}
				}
			}
		}
...
*/	
    ...
    function get_selected_field($id=false){
        // Fetch pages without parents
        $array = array();
		$check = $this->comman_model->get_by('products_field',array('product_id'=>$id),false,false,false);
		if($check){
			foreach($check as $n){
                $array[] = $n->value_id;
			}
