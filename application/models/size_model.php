...
/*            $this->rules_lang["body_$key"] = array('field'=>"body_$key", 'label'=>'lang:Body', 'rules'=>'trim');
    ...
    public function get_new()
	{
        ...
        {
            $sizes->{"title_$key"} = '';
            $sizes->{"navigation_title_$key"} = '';
            //$page->{"slug_$key"} = '';
            $sizes->{"body_$key"} = '';
            $sizes->{"keywords_$key"} = '';
            $sizes->{"description_$key"} = '';
        }
	...
	{
		if (count($data)) {
			foreach ($data as $order => $sizes) {
				if ($sizes['item_id'] != '') {
					$data = array('parent_id' => (int) $sizes['parent_id'], 'order' => $order);
					$this->db->set($data)->where($this->_primary_key, $sizes['item_id'])->update($this->_table_name);
				}
			}
		}
	}
	...
	{
		$this->db->select('sizes.*, p.slug as parent_slug, p.title as parent_title');
		$this->db->join('sizes as p', 'sizes.parent_id=p.id', 'left');
...
/*        $templatesDirectory = opendir(APPPATH.'../templates/'.$CI->app_settings['template']);
	...
	public function get_lang($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.size_id');
    ...
    public function get_lang_array($id = NULL, $single = FALSE, $lang_id=1)
    {
        ...
        if($id != NULL)
        {
            $result = $this->db
                    ...
                    ->where(array('sizes_lang.language_id'=>$lang_id,'sizes.id'=>$id))
                    ->join('sizes_lang','sizes.id = sizes_lang.size_id')
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        // Save lang data
        $this->db->delete($this->_table_name.'_lang', array('size_id' => $id));
    ...
    public function get_id_by_name ($size_id)
    {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->join($this->_table_name.'_lang', $this->_table_name.'.id = '.$this->_table_name.'_lang.size_id');
		...
		foreach ($sizes as $n) {
		  if(url_title_cro($n['title'], '-', TRUE) == $size_id)  
          {
            return $n['id'];
          }
		}
     ...
     public function get_nested_services($lang_id=2)
    {
        ...
        $sizes = $this->db->get($this->_table_name)->result();
    ...
    {
        $this->db->delete('sizes_lang', array('size_id' => $id)); 
		$this->db->delete($this->_table_name, array('id'=>$id));
        $this->db->delete('product_size', array('size_id' => $id)); 
        
		// Reset parent ID for its children
		//$this->db->set(array('size_id' => 0))->where('size_id', $id)->update('products');
    }
