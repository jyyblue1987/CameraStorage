...
/*                'medium' => array(
	...
	protected function filter_opacity(&$img, $opacity) 
	{
		if (!isset($opacity)) {
			return false;
		}
	...
	protected function create_scaled_image($file_name, $version, $options) {
		...
		$wtrmrk_file = "assets/uploads/ecity-wm.png";
		$watermark = @imagecreatefrompng($wtrmrk_file);
		
		// Set the tile
		@imagesettile($new_img, $watermark);

		// Make the image repeat
		@imagefilledrectangle($new_img, 0, 0, 1000, 1000, IMG_COLOR_TILED);
		
		$this->filter_opacity($watermark, 75);
		...
		@imagecopy($new_img, $watermark, $dst_x, $dst_y, 0, 0, $wtrmrk_w, $wtrmrk_h);		
		imagejpeg($new_img, $new_file_path, 100);  		
    ...
    protected function trim_file_name($name, $type, $index, $content_range) {
        ...
        if (strpos($name, '.') === false &&(preg_match('/^image\/(gif|jpe?g|png)/', $type, $matches)||preg_match('/^video\/(mp4|mov)/', $type, $matches))) {
